...
const $$HeroCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="relative overflow-hidden rounded-xl border bg-white transition-transform duration-500 hover:-translate-y-1 hover:scale-105 md:flex md:flex-row dark:border-neutral-600 dark:bg-neutral-900"> <a${addAttribute(`/posts/${post.slug}/`, "href")} class="block md:flex md:flex-1"> <div class="md:w-2/3"> ${renderComponent($$result, "OptimizedCover", $$OptimizedPicture, { "src": post.data.cover, "alt": post.data.coverAlt })} </div> <div class="absolute bottom-0 end-0 start-0 bg-gradient-to-t from-black p-4 md:relative md:flex md:w-1/3 md:flex-col md:justify-start md:bg-transparent md:p-5"> <div class="mt-8 flex items-center gap-4 text-xs text-white md:text-neutral-900 dark:text-white"> <span>${post.data.tags.join(" ")}</span> ${renderComponent($$result, "FormattedDate", $$FormattedDate, { "date": post.data.pubDate })} </div> <h3 class="mt-3 text-lg font-bold text-white md:text-neutral-900 dark:text-white"> ${post.data.title} </h3> <p class="mt-3 text-white md:text-neutral-900 dark:text-white"> ${truncatedDescription} </p> </div> </a> </div>`;
