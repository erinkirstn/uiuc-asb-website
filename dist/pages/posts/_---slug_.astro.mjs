...
const $$RelatedPostsCard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="grid grid-cols-1 gap-4 md:grid-cols-3"> ${relatedPosts.map(async (post) => renderTemplate`<div class="relative mb-4 overflow-hidden rounded-xl bg-white transition-transform duration-500 hover:-translate-y-1 hover:scale-105 dark:bg-neutral-900"> <a${addAttribute(`/posts/${post.slug}/`, "href")}> ${renderComponent($$result, "OptimizedCover", $$OptimizedPicture, { "src": (await import(post.data.cover)).default, "alt": post.data.coverAlt })} <div class="absolute bottom-0 end-0 start-0 bg-gradient-to-t p-4 md:p-5"> <div class="mt-16 flex items-center gap-4 text-xs text-white"> <span>${post.data.tags.join(" ")}</span> ${renderComponent($$result, "FormattedDate", $$FormattedDate, { "date": post.data.pubDate })} </div> <h2 class="mt-2 text-lg font-bold text-white">${post.data.title}</h2> </div> </a> </div>`)} </div>`;
